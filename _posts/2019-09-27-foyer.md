...
have overriden this function with `Foyer.Forcefield.registerAtomType`, which is
...
is this GRAMMAR string, but not we've formatted the string
...
Lastly, we have a property for the `self.PARSER`, which is utilizes
...
we are already done. 
On addition, we erase the positions only to re-add them later.
...
First, `_prepare_atoms` involves initiialzing `typemap` properties and
...
This is a lot of if-statements, analyzing the expressions as
...
we will end up caling `atom_expr_matches` a bunch to dig
...
The technical jargon is we are perform [**subgraph isomorphisms**](https://en.wikipedia.org/wiki/Subgraph_isomorphism_problem).
...
when we call `rule.find_matches`, where the subraph matching code is used
...
said to which bonded entity had that parameter information.
...
the refernce length, and the force constant.
...
HarmonicBondForce, and put sthem in the second HarmonicBondForce.
...
parameters, or poorly-defined bonding, poorly-defined particle names, 
among others.
...
mBuild also has additional formats that extra information from the
...
I advise others who truly want to understand the package
