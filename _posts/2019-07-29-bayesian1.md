...
### (7/29) Checkpoint: maybe summarizing things so far
...
### (7/30) Update: I was pretty wrong yesterday
...
In this example, we are modelling the relationship between price and age
...
molecular modelling, but now I probably will.
...
molecular modellers will probably cringe that we aren't using $r$ and $v$ or 
...
in molecular modelling the derivativative of energy WRT position is our force.

At this point, we have some coupled, partial differential equations.
All molecular modellers are familiar with this - we just need to numerically
...
Now here's where the molecular modellers might finally have a leg-up,
...
(I'm speaking from molecular modelling experience, 
